  ...
  backend: {
    occ: {
      endpoints: {
        ...
        productSearch:
          'products/search?fields=products(code,name,summary,price(FULL),images(DEFAULT),stock(FULL),averageRating,poSpecCharValueUses(FULL),variantOptions,productSpecification,productOfferingPrice(FULL)),facets,breadcrumbs,pagination(DEFAULT),sorts(DEFAULT),freeTextSearch,currentQuery',
        // tslint:enable
