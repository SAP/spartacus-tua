...
import {
  ...
  TmaSelfcareSubscriptionsTreeNode,
  TREE_TOGGLE
} from '../../../../model';
...
export class TmfSelfcareTreeService {
   ...
   */
  initialize(root: TmaSelfcareSubscriptions, activeId: string): void {
    if (activeId) {
  ...
  toggle(product: TmaSelfcareSubscriptionsTreeNode) {
  ...
  protected expandUntilActiveNode(
    node: TmaSelfcareSubscriptions,
    activeUnitId: string
  ) {
    const hasActiveChild = (n: any, id: string): boolean =>
      !!n.product.find(
        (child: TmaSelfcareSubscriptionsTreeNode) =>
