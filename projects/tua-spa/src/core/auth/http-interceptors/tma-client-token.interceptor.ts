...
import { AuthService, ClientToken, InterceptorUtil, OccEndpointsService, USE_CLIENT_TOKEN } from '@spartacus/core';
...
export class TmaClientTokenInterceptor implements HttpInterceptor {
  constructor(
  ...
  intercept(
    ...
    this.authService.isUserLoggedIn().subscribe(value => isUserLoggedIn = value);
    return this.getClientToken(request).pipe(
      take(1),
      switchMap((token: ClientToken) => {
        if (!isUserLoggedIn && (!token || !token.token_type || !token.access_token)) {
          this.authService.getClientToken().subscribe(value => token = value);
        }
        if (
