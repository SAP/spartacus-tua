...
export class TmaPriceDisplayComponent implements OnInit {
  ...
  ngOnInit() {
    this.list.push(this.price.bundledPop[0]);
  }
  ...
  getSortedList(list:TmaProductOfferingPrice[]): TmaProductOfferingPrice[] {
    ...
    recurringCharges.sort((a, b) => (Number(a.cycle.cycleStart) > Number(b.cycle.cycleStart)) ? 1 : -1).forEach((bundlePop:TmaProductOfferingPrice)=> newList.push(bundlePop));
    usageCharges.sort((x, y) =>
      (x.unitOfMeasure > y.unitOfMeasure ? 1 : (x.unitOfMeasure === y.unitOfMeasure ?
        ((Number(x.tierStart) > Number(y.tierStart) || x.tierStart === undefined) ? 1 : -1) : -1))).forEach((bundlePop:TmaProductOfferingPrice)=> newList.push(bundlePop));
