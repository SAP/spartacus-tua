...
export class TmaGuidedSellingCurrentSelectionComponent implements OnInit, OnDestroy {
   ...
   */
  getPayNowPrice(price: TmaProductOfferingPrice[]): TmaProductOfferingPrice[] | string {
    if (!price || price.length === 0) {
      return CURRENT_SELECTION.DASH;
    }
    this.priceValue = this.priceService.getAllPriceList(price[0]);
    const oneTimePrices: TmaProductOfferingPrice[] = this.priceService.getPayNowPrices(this.priceValue);
   ...
   * @return String containing the formatted price
   */
  getRecurringPrice(price: TmaProductOfferingPrice[]): TmaProductOfferingPrice[] | string {
    if (!price || price.length === 0 || !price[0].bundledPop) {
      return CURRENT_SELECTION.DASH;
    }

    this.priceValue = this.priceService.getAllPriceList(price[0]);
    const recurringPrices: TmaProductOfferingPrice[] = this.priceService.getRecurringPrices(this.priceValue);
