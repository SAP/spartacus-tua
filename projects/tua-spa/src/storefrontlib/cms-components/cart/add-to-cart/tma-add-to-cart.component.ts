...
import {distinctUntilChanged, filter, first, last, take, takeUntil, tap} from 'rxjs/operators';
import {
  Appointment,
  AppointmentService,
...
export class TmaAddToCartComponent
  ...
  addSpoToCart(checklistActions?: TmaChecklistAction[]): void {
    this.configurablePscvusService.addToCartPressed = true;
  ...
  addServicableProductToCart(quantity: number) {
    ...
    if (sessionAddress.relatedParty) {
      sessionAddress.relatedParty.id =
   ...
   */
  addBpoToCart(installationAddress: string): void {
  ...
  /**
   * Check for mandatory product
   * @param po TmaProduct
   * @returns true/false
   */
  checkPoForMandatoryPscvu(po: TmaProduct): boolean {
    return (
      po?.poSpecCharValueUses &&
      po?.poSpecCharValueUses?.length > 0 &&
      po?.poSpecCharValueUses?.some((pscvu: TmaProductSpecCharValueUse) => {
        if (pscvu?.minCardinality === 0) {
          return false;
        }

        if (
          pscvu?.minCardinality === 1 &&
          pscvu?.maxCardinality === 1 &&
          pscvu?.productSpecCharacteristicValue?.length === 1 &&
          pscvu?.productSpecCharacteristicValue[0]?.value !== '#'
        ) {
          return false;
        }
   ...
   * @param checklistActions list of {@link TmaChecklistAction}
   */
  protected addCartEntryWithChecklistActions(
    checklistActions: TmaChecklistAction[],
    quantity: number
  ): void {
    ...
    if (msisdnRequired) {
      ...
      msisdn$ = this.logicalResourceReservationService
      ...
      msisdn$.subscribe((result: Reservation) => {
        if (
      ...
      if (Object.keys(values).length > 0) {
        ...
        values.forEach(
          (value: Appointment | GeographicAddress | Reservation) => {
            ...
            if (
              value['@type'] === MSISDN_RESERVATION.RESOURCE_ITEM_RESERVATION
            ) {
